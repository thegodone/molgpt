...
if __name__ == '__main__':
        ...
        model.load_state_dict(torch.load(args.model_weights_dir + args.model_weight, map_location=torch.device(device)),strict=False)
        ...
        print(results.head)
        print('results are saved :',args.gen_csv_dir + args.csv_name + '.csv')
        ...
        print('F Valid ratio: ', np.round(len(results)/(args.batch_size*gen_iter*count), 3))
        print('F Unique ratio: ', np.round(len(unique_smiles)/len(results), 3))
        print('F Novelty ratio: ', np.round(novel_ratio/100, 3))
